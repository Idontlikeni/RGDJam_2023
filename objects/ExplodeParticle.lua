ExplodeParticle = GameObject:extend()

function ExplodeParticle:new(area, x, y, opts)
    ExplodeParticle.super.new(self, area, x, y, opts)

    self.color = opts.color or default_color
    self.r = math.random(0, 2*math.pi)
    self.s = opts.s or love.math.random(2, 3)
    self.v = opts.v or love.math.random(0.5, 2)
    self.line_width = 4
    self.timer:tween(opts.d or love.math.random(0.3, 0.5), self, {s = 0, v = 0, line_width = 0}, 'linear', function ()
        self.dead = true
    end)
    --self.collider:setLinearVelocity(self.v*math.cos(self.r), self.v*math.sin(self.r))
end

function ExplodeParticle:update(dt)
    ExplodeParticle.super.update(self, dt)
    self.x, self.y = self.x + self.v * math.cos(self.r), self.y + self.v * math.sin(self.r)
end

function ExplodeParticle:draw()
    pushRotate(self.x, self.y, self.r)
    love.graphics.setLineWidth(self.line_width)
    love.graphics.setColor(self.color)
    love.graphics.line(self.x - self.s, self.y, self.x + self.s, self.y)
    love.graphics.setColor(default_color)
    love.graphics.setLineWidth(1)
    love.graphics.pop()
end